...
def check_premium_code(code):
    ...
    if user.balance >= 250.0:
        ...
        deposit_needed = 250.0 - user.balance
    
    return jsonify({
        'booking': booking.to_dict(include_selections=show_selections),
        'has_access': has_access,
        'requires_deposit': requires_deposit,
        'user_balance_usd': round(user.balance, 2),
        'minimum_balance_required': 250.0,
        'deposit_needed': max(0, 250.0 - user.balance) if user.balance < 250.0 else 0
    }), 200
    ...
    try:
        ...
        if user.balance >= 250.0:
            ...
            return jsonify({
                ...
                'your_balance_usd': round(user.balance, 2),
...
def place_bet_from_premium(code):
    ...
    try:
        ...
        # NEW LOGIC: Allow users with sufficient balance to bet
        # No purchase required - balance-based access system
        # Check if user has minimum balance for premium access
        if user.balance < 250.0:
            return jsonify({
                'error': 'Insufficient balance for premium betting',
        ...
        if user.balance < stake_usd:
            return jsonify({
                'error': 'Insufficient balance',
                'required_usd': stake_usd,
                'current_balance': user.balance
            }), 400
        ...
        potential_payout_usd = stake_usd * booking.total_odds
        ...
        bet = Bet(
            ...
            potential_payout=potential_payout_usd,
            bet_type='sports',  # Premium bets are sports bets
            status='active',  # Changed from pending to active
            booking_code=booking.booking_code
        )
        
        # Deduct stake from user balance (in USD)
        user.balance -= stake_usd
